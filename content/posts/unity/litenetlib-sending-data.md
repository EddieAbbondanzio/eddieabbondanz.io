...
Continuing off the previous post about [how to build a basic server / client set up with LiteNetLib]({{< ref "/posts/unity/litenetlib-basic-server-client.md" >}}) it's time to talk about sending data. Because, well, a network set up that doesn't actually send any data is kind of useless.
...
In the `NetworkReceiveEvent` delegate we're given a few parameters. The `NetPeer` that the message came from, a `NetDataReader` for reading the incoming data, and the `DeliveryMethod` of how it was sent (see [LiteNetLib's Delivery Methods]({{< ref "/posts/unity/litenetlib-delivery-methods.md" >}})).
...
{{% figure src="/img/unity/litenetlib-sending-data/1.png" %}}
